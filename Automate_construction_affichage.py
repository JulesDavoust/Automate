...
def matrice_auto(matrice, nbr_symb, nbr_etats, I, T, Q, nvline1, tab_decomp_transition, etats_term_num, etats_init_num):
    ...
    for line in range(nbr_etats):
        ...
        for col in range(nbr_symb + 2):
            ...
            if (col != 0 and col != 1):
                transi_nv = nvline[1]
                #print("tab_dt :", tab_decomp_transition)
...
def construction_automate(text):
    ...
    while (cpt <= leng_auto - 1):
        ...
        if ((ord("a") < ord(tab_tri[check]) or ord(tab_tri[check]) < ord("z")) or ord(tab_tri[check]) == ord("*")):
            ...
            # print("c2 :",check2)
            # print(len(tab_tri))
            # print("tabtri check2 :", tab_tri[check2])
            ...
            tab_decomp.append(var_recup)
        # recup_tab_decomp(tab_decomp)
        # print("tab_decomp :", tab_decomp)
        tab_decomp_transition.append(tab_decomp)
        # print(tab_tri)
        cpt = cpt + 1
    # print(tab_decomp_transition)
    #print(tab_decomp_transition)
    for i in range(len(tab_decomp_transition)):
        for j in range(3):
            # if (ord("a") > ord(tab_decomp_transition[i][j]) or ord(tab_decomp_transition[i][j]) > ord("z")) and ord(
            #        tab_decomp_transition[i][j]) != ord("*"):
    ...
    matrice_auto(matrice, nbr_symb_bis, nbr_etats, I, T, Q, nvline1, tab_decomp_transition, nombre_etats_term,
                 nombre_etats_init)
    #print(matrice)
    ...
    file.close()
    return nbr_symb, A, nbr_etats, Q, etats_init_num, nombre_etats_init, I, etats_term_num, nombre_etats_term, T, nbr_transitions, epsilon, row_labels, column_labels, matrice, automate
            ...
            if comp == True:
                ...
                if(deter == False and asynch == True):
                    ...
                    if (val_comp == "O"):
                        ...
                        matrice_cp[len(matrice_cp)-1].insert(0, "P")
